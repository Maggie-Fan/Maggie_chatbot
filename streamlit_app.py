...
# User name for title (you can customize this)
user_name = "Maggie"
...
for message in st.session_state.messages:
    with st_c_chat.chat_message(message["role"]):
...
def chat(prompt: str):
    ...
    with st_c_chat.chat_message("user"):
        st.markdown(prompt)

    # 產生並顯示回應
    response = generate_response(prompt)
    st.session_state.messages.append({"role": "assistant", "content": response})
    with st_c_chat.chat_message("assistant"):
